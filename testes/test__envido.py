...
def test_controlador_envido(envido,dados, cbr, jogador, jogadorBot2, mocker):
    """Testa se os métodos corretos são chamados"""
    #estado_atual !=0 ou tipo == estado_atual
    assert envido.controlador_envido(cbr, dados, 0, 1, jogador, jogadorBot2, Interface()) == None
    envido.estado_atual = 1
    assert envido.controlador_envido(cbr, dados, 0, 1, jogador, jogadorBot2, Interface()) == None
    # quem_pediu = jogador_bloqueado
    envido.estado_atual = 0
    assert envido.controlador_envido(cbr, dados, 1, 0, jogador, jogadorBot2, Interface()) == None
    # else
    ...
    envido.controlador_envido(cbr, dados, 6, 1, jogador, jogadorBot2, Interface())
    method1_mock.assert_called_with(1)
    method2_mock.assert_called_with(jogador, jogadorBot2)
    #tipo 6
    method3_mock.assert_called_with(cbr, 1, jogador, jogadorBot2)
    #tipo 7
    envido.controlador_envido(cbr, dados, 7, 1, jogador, jogadorBot2, Interface())
    method4_mock.assert_called_with(cbr, 1, jogador, jogadorBot2)
    #tipo 8
    envido.controlador_envido(cbr,dados, 8, 1, jogador, jogadorBot2, Interface())
    method5_mock.assert_called_with(cbr, 1, jogador, jogadorBot2)
    #quem fugiu = 0
    method6_mock.assert_called_with(0, jogador.nome, 0, jogadorBot2.nome, 0)

def test_envido(envido, cbr, jogador, jogadorBot2, monkeypatch):
    """Testa se o envido é realizado corretamente"""

    #humano pediu envido
    with patch.object(envido,'avaliar_vencedor_envido') as mock:
        envido.envido(cbr, 1, jogador, jogadorBot2)
    mock.assert_called_with(1, jogador, jogadorBot2)
    #robo pediu envido
        #humano foge
    monkeypatch.setattr('builtins.input', lambda _: 0)
    envido.envido(cbr, 2, jogador, jogadorBot2)
    assert jogadorBot2.pontos == 1
    assert envido.quem_fugiu == 1
        #humano aceita
    monkeypatch.setattr('builtins.input', lambda _: 1)
    with patch.object(envido,'avaliar_vencedor_envido') as mock:
        envido.envido(cbr, 2, jogador, jogadorBot2)
    mock.assert_called_with(2, jogador, jogadorBot2)
        # humano pede real envido
    monkeypatch.setattr('builtins.input', lambda _: 2)
    envido.jogador_bloqueado = 1
    with patch.object(envido, 'real_envido') as mock:
        envido.envido(cbr, 2, jogador, jogadorBot2)
    mock.assert_called_with(cbr, 2, jogador, jogadorBot2)
...
def test_real_envido(envido, cbr, jogador, jogadorBot2, mocker, monkeypatch):
    ...
    envido.real_envido(cbr, 1, jogador, jogadorBot2)
    method1_mock.assert_called_with(cbr, 7, 1,envido.jogador2_pontos)
    method2_mock.assert_called_with(1, jogador, jogadorBot2)
    #robo pediu real envido
        # humano fugiu
    monkeypatch.setattr('builtins.input', lambda _: 0)
    envido.real_envido(cbr, 2, jogador, jogadorBot2)
    assert jogadorBot2.pontos == 2
    assert envido.quem_fugiu == 1
        # humano aceitou
    monkeypatch.setattr('builtins.input', lambda _: 1)
    envido.real_envido(cbr, 2, jogador, jogadorBot2)
    method2_mock.assert_called_with(2, jogador, jogadorBot2)
        # humano pediu falta envido
    envido.jogador_bloqueado = 2
    monkeypatch.setattr('builtins.input', lambda _: 2)
    with patch.object(envido, 'falta_envido') as mock:
        envido.real_envido(cbr, 2, jogador, jogadorBot2)
    mock.assert_called_with(cbr, 1, jogador, jogadorBot2)

def test_falta_envido(cbr, envido, jogador, jogadorBot2, mocker, monkeypatch):
    ...
    # humano pediu falta envido
    envido.falta_envido(cbr, 1, jogador, jogadorBot2)
    method1_mock.assert_called_with(cbr, 8, 1, envido.jogador2_pontos)
    method2_mock.assert_called_with(1, jogador, jogadorBot2)
    # bot pediu falta envido
        # humano fugiu
    monkeypatch.setattr('builtins.input', lambda _: 0)
    retorno = envido.falta_envido(cbr, 2, jogador, jogadorBot2)
    assert jogadorBot2.pontos == 5
    assert envido.quem_fugiu == 1
    assert retorno == False
        # humano aceitou
    monkeypatch.setattr('builtins.input', lambda _: 1)
    envido.falta_envido(cbr, 2, jogador, jogadorBot2)
    method2_mock.assert_called_with(2, jogador, jogadorBot2)

def test_avaliar_vencedor_envido(envido, jogador, jogadorBot2):
    """Testa se o vencedor do envido está sendo definido corretamente"""
    # humano >= robo
    envido.jogador1_pontos = 1
    envido.valor_envido = 1
    envido.avaliar_vencedor_envido(1, jogador, jogadorBot2)
...
def test_avaliar_vencedor_falta_envido(envido, jogador, jogadorBot2):
    """Testa se o vencedor do falta envido está sendo definido corretamente"""
    # humano >= robo
    envido.jogador1_pontos = 1
    envido.valor_envido = 1
    envido.avaliar_vencedor_falta_envido(1, jogador, jogadorBot2)
    assert jogador.pontos == 1
    assert envido.quem_venceu_envido == 1
        # robo > humano
    envido.jogador2_pontos = 2
...
def test_definir_pontos_jogadores(envido, jogador, jogadorBot2):
    """Testa se os pontos de envido dos jogadores estão sendo salvos corretamente"""
    jogador.envido = 5
    jogadorBot2.envido = 7
    envido.definir_pontos_jogadores(jogador, jogadorBot2)
